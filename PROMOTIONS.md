...
| `deploy-verify` | Skill | `~/.claude/skills/deploy-verify` | 2026-09-24: new. Generic pre-deploy gate, deploy, verify loop and Deploy Log; reads a per-project `docs/ops/deploy-verification.md`. The Co-Pilot's checklist is not written yet (it is a write to the product repo). Follows the minimum-viable-factory `deploy-checklist`, generalized. |
| `legacy-audit` | Skill | `~/.claude/skills/legacy-audit` | 2026-09-24: new. Reverse-first audit method from the legacy-codebase lecture. The lecture's reference repository was not read. A trigger smoke check ran against it; no output-quality eval yet. |
| `plan-adherence` | Skill | `~/.claude/skills/plan-adherence` | Saves a precise multi-step spec verbatim before any planning. Row added 2026-09-24 (missing before); 0 recorded invocations, so keep only if Austin milestone M1 shows use. |
| `session-handoff` | Skill | `~/.claude/skills/session-handoff` | Writes a handoff document and starter prompt for a fresh session. Row added 2026-09-24 (missing before); used for the 2026-09-24 skills-overhaul handoff. |
| `langfuse` | Skill (third party) | `~/.claude/skills/langfuse` (symlink into the `langfuse-skills` clone) | Upstream Langfuse skill bundle, unchanged; excluded from the skills repository. Row added 2026-09-24 (missing before). |
| `eval-gate-triage` | Skill (project-local) | `openemr-base-clean/.claude/skills` and `.agents/skills` | Lives in the Co-Pilot product repo; its two copies differ only in host wording. Row added 2026-09-24; consolidation waits until that repo is idle. |
| Jev routing client (shadow) | Harness | `factory/routing/jev_client.py`, `factory/routing/jev-policy.yaml` | 2026-09-24: policy file and client with recorded-response tests; not wired into anything. Uses the `OPENROUTER_API_KEY` environment variable by name only. Request body shape unverified until the first shadow call. |
